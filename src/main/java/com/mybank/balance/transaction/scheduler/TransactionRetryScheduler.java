...
public class TransactionRetryScheduler {
    ...
    /**
     * 1分钟+随机30秒 间隔的定时任务，定时查询 transactions status=“RETRY” 或者 “PENGDING” ,"PROCESSING" 且 now - 5min > updated 的记录
     * 重新处理交易：
     ...
     * c:释放分布式锁
     */
    // 每 1 分钟 + 随机 0~30 秒执行一次（单位：毫秒）
    @Scheduled(fixedDelayString = "#{60000 + T(java.util.concurrent.ThreadLocalRandom).current().nextInt(30000)}")
